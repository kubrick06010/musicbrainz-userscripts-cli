...
//
// Scenario A (anonymous works, chaban's case): album/get answers 200 (the real
// response body from the #418 HAR) → the 13 ISRCs must import, the request must
// carry NO X-User-Auth-Token, and no login error may appear.
...
async function run(qobuzStatus, withToken = false) {
  const page = await ctx.newPage();
  const errs = []; page.on('pageerror', e => errs.push(e.message));
  await page.addInitScript(({ fixture, qobuzStatus, withToken }) => {
    window.__qbRequests = [];
    ...
    window.GM_xmlhttpRequest = (o) => {
      ...
      if (/qobuz\.com\/api\.json/.test(o.url)) {
        ...
        // a session request always succeeds; anonymous follows qobuzStatus (geo simulation)
        const ok = (o.headers && o.headers['X-User-Auth-Token']) || qobuzStatus === 200;
        return done(ok ? 200 : qobuzStatus, ok ? fixture : '{"status":"error","code":404,"message":"No result matching given argument"}');
      }
...
console.log('── Scenario C: geo-blocked 404 but PC session present (the old #353 path) ──');
const C = await run(404, true);
...
ck(cq.length === 2, 'anonymous try then session retry (2 album/get calls)');
ck(cq[0] && !('X-User-Auth-Token' in cq[0].headers) && cq[1] && cq[1].headers['X-User-Auth-Token'] === 'fake-session-token', 'first call anonymous, second carries the session token');
ck(/Qobuz album "Move Positive": 13 track/i.test(C.out.log), 'session retry delivered the 13 tracks');
