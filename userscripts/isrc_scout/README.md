...
- **ISRC provider menu (▾ next to ⟳ SoundExchange)** — the per-track buttons and the bulk button are one *track ISRC provider* selector. SoundExchange is the default; the **▾** lists every other provider available for the release (Deezer / Spotify / Beatport / Tidal / Volumo / HDtracks — whichever have a link in MB or one Platform Check found). Picking one re-skins **all** the per-track buttons to that provider's icon (the choice is global for the release and **not remembered** — it resets to SoundExchange on reload). With an album provider selected, a per-track button fetches that album once and fills/verifies just that one track's ISRC; the bulk button imports the whole album.
