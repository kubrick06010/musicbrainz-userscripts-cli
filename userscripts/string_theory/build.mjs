...
const outsideCode = (md, fn) => md.split(/(```[\s\S]*?```|~~~[\s\S]*?~~~)/g).map((seg, i) => (i % 2 ? seg : fn(seg))).join('');
const demote      = md => md.replace(/^(#{1,6})(\s)/gm, (_m, h, s) => (h.length < 6 ? '#' + h : h) + s);
function rewriteLinks(md, member) {
  // leave absolute URLs, anchors, root- and parent-relative paths; re-point the rest at ../<member>/
  const fix = p => /^(https?:|mailto:|#|\/|\.\.\/)/i.test(p) ? p : `../${member}/${p.replace(/^\.\//, '')}`;
  return md
    .replace(/\]\(([^)\s]+)(\s+[^)]*)?\)/g, (_m, p, t) => `](${fix(p)}${t || ''})`)   // markdown ](path) / ](path "title")
    .replace(/\b(src|href)=("|')([^"']+)\2/gi, (_m, a, q, p) => `${a}=${q}${fix(p)}${q}`);   // html src/href
}
const sections = [];
for (const n of MEMBERS) {
  ...
  const title = stripImg(h1 ? h1[1] : n) || n;
  md = md.replace(/^#\s+.+\n?/m, '');                                   // drop the member's own H1 (re-added as an H2 below)
  const body = outsideCode(md, seg => rewriteLinks(demote(seg), n)).trim();
