...
| **Qobuz** | Composer, Lyricist, Producer, Publisher, performers | **names only** — Qobuz exposes no artist/profile links on credits, so each name is resolved by MB **name search + your review** | authenticated `album/get` when signed in to Qobuz via [Platform Check](../platform_check/README.md) (reliable, geo-independent); otherwise the server-rendered store page | optional [Qobuz login](../platform_check/README.md) — same roled credits either way, the login just makes the fetch reliable |
