...
export async function dispatchAllRelationships(companies, artistRoles, tracklistRels, applyToTracks, createWorksMode, discogsTracklist, processTracklist, resolvedEntityTypes, confirmedMap, discogsUrl, dedupOpts) {
    ...
    const stagedPart = existedStaged > 0
        ? `, ${existedStaged} already added this session`
        : '';
    const selNote = recSelectionActive ? ` — applied to ${checkedRecGids.size} of ${recordingByGid.size} selected recording(s)` : '';   // #113: mark it in the header result (this line mirrors to the toolbar status)
