...
// `src/credit_hoarder.user.js`  — entry module for esbuild (eventually one
// of many `src/*.js` modules; the multi-file split lands incrementally per
// issue #32).
//
// Output: meta.txt + esbuild IIFE bundle → `dist/credit_hoarder.user.js`.
//
...
//   - Serves the built script on http://127.0.0.1:8765/credit_hoarder.user.js
...
const OUT      = 'dist/credit_hoarder.user.js';
...
function devifyMetadata(meta) {
    const localUrl = `http://${HOST}:${PORT}/credit_hoarder.user.js`;
...
function serve() {
    ...
    server.listen(PORT, HOST, () => {
        const url = `http://${HOST}:${PORT}/credit_hoarder.user.js`;
