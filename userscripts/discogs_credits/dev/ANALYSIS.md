...
| # | Bug | Fix sketch |
| --- | --- | --- |
| #2 | Mastering relationship deprecated for recordings | When `applyToTracks` is on, skip link types whose `(linkTypeID, artist, recording)` triple is invalid for the artist→recording direction. Keep them at release level only. Generalize: validate every dispatch against `MB.linkedEntities.link_type` before dispatching, log a warning if invalid, never dispatch invalid. |
