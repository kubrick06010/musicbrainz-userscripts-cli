...
// of being unconditionally hardcoded. Defaults are OFF (nothing hidden) — verifies that,
// checking one via the settings panel hides it live (no reload), and the choice persists
// across a fresh page load via localStorage.
...
// 1) default: nothing hidden (off by default)
...
ck(afterCheck.player === true && afterCheck.tags === true, `the other two groups stay visible — this option is independent (got player=${afterCheck.player}, tags=${afterCheck.tags})`);
...
ck(afterReload.player === true && afterReload.tags === true, 'the untouched options still default to visible after reload');
