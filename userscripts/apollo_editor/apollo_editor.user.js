...
(function () {
  ...
  const VERSION = '2026.7.3.190135';   // keep in sync with @version (fallback when GM_info is unavailable under @grant none)
    ...
    tbl.addEventListener('contextmenu', e => {
      ...
      if (e.target.closest('td.tc-tka')) {
        ...
        const setArtistFromRec = (m, i) => {
          ...
          if (acText(u(t.artistCredit)) === acText(recAc)) return;   // already the same
