...
(function () {
  ...
  const VERSION = '2026.6.16.134500';   // keep in sync with @version (fallback when GM_info is unavailable under @grant none)
  ...
  async function rebuild(noMatch) {
    ...
    rerender();
    if (!noMatch && SETTINGS.autoMatch !== false) await matchModel(); else tagDiscogsForAll();   // #227
