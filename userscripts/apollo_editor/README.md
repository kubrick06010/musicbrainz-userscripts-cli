...
- **Recordings editor**
    ...
    - **Confusable / invisible characters** — wherever a **title or artist** is shown (both tables, the recording picker — **not** only inside a detailed-highlight diff) a curly `’`, an en/em dash, a no-break or zero-width space, a tab, etc. is **enlarged**, names its exact codepoint on hover, and draws invisibles as a visible glyph, so look-alike or hidden characters are obvious. Size is the _Appearance → Enlarge punctuation by N px_ setting (`0` = off, the master switch); plain ASCII `'` `"` `-` are left alone outside a diff so normal text isn't littered.
