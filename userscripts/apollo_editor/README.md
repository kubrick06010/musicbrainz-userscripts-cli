...
- **Recordings editor**
    ...
    - **Detailed highlighting** (opt-in, _Matching_ settings) — highlights the exact **differing characters** in a mismatching **title and artist** (including a casing- or punctuation-only difference the match would otherwise tolerate), instead of the whole field, and shades a **length mismatch** by how large the gap is (faint under a second → solid red past five). **Confusable characters** in a difference — a straight `'` vs a curly `’`, a hyphen vs an en/em dash, a no-break or zero-width space — are boxed with a dashed outline and name the exact codepoint on hover (invisibles render as a visible glyph), so look-alike differences are obvious.
