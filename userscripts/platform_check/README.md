...
| Qobuz       | DuckDuckGo / Brave (`site:qobuz.com … /album/`)     | store-page scrape (JSON-LD + track markers) | — |
...
1. **Qobuz** has no usable unauthenticated API (its `api.json` catalogue endpoints return 401 even with a freshly-scraped `app_id`), but its public **store page is fully server-rendered**. So Qobuz resolves by web search (`site:qobuz.com … /album/`) — or an existing MB rel — and is verified by scraping that page: track count from the per-track add-to-cart markers (the page renders an empty duplicate of every track row, so the `track__info` blocks can't be counted directly), album name + year from the JSON-LD `MusicAlbum` block, label from the label link, and artist from `og:title`. It exposes **no barcode** (the page carries no UPC) and **no format** (digital-only), so it's judged on track count + title like the other streaming providers.
